...
Numary Ledger has robust support for working in different currencies. Let's go a little deeper into how that works.
...
This Numscript instructs Numary Ledger to transfer 100 coins to the player `benwyatt`. Notice that we specify the currency and amount together, and that we specify the currency as `COIN` singular. All amounts in Numscript are always specified in this way.

## Divisible currencies

However, lately, the economy of Cones of Dunshire has been undergoing significant deflation, and thus coins have been appreciating in value. Because they're indivisble, small transactions among players have become difficult, and they've had to resort to making change by bartering with clumps of dirt. So the game administration has decided to introduce a new currency, the piece-of-ten (designated as `DSP` for "<u>D</u>un<u>s</u>hire <u>P</u>iece-of-Ten"), at a lower valuation. This new currency will initially trade at a rate of 10 pieces-of-ten to the coin, and as its name suggests, will itself be divisible into 10 sub-units (colloquially called "pieces-of-one" or just "pieces" for short).

Let's suppose that our intrepid player `benwyatt` has completed a new task for the ledgerman, but this time he's going to get paid in 1,000 pieces-of-ten instead of 100 coin. Here's what the Numscript would look like:
...
You might find yourself asking: Why not use a decimal representation? Why not `[DSP 5.2]` or `[EUR 5.23]`? The answer is, that [floating point numbers are too imprecise for finance](https://www.youtube.com/watch?v=yZjCQ3T5yXo). Numscript and Numary Ledger avoid this problem by only using integer math.
...
:::info Dig deeper
Want to learn more about how Numary Ledger does integer math? [The reference docs](/oss/ledger/reference/numscript/rounding) have you covered!
:::
