...
Once you've run that transaction succesfully, let's have a look at [`leslieknope`'s balance'](https://control.numary.com/accounts/player:leslieknope) to find that transaction. It should be the topmost one listed. Click the details arrow to the right of the transaction to get more details.
...
:::info There's much more to metadata

Numary Ledger allows you to also add metadata to accounts. This metadata can be used to construct much more sophisticated transactions, with variables that bind to specific metadata at execution time. At the moment, we don't have documentation for how to use these feature, but expect them soon.
