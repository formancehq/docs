...
From version v2.0.0-rc.27, CRDs are now packaged with `helm.sh/resource-policy: keep` to avoid deletion of the CRDs when the operator is uninstalled.
...
### Migrating from Operator with CRDs to separated CRDs chart
...
```bash
# Example for authclients.formance.com CRD

kubectl annotate crd authclients.formance.com meta.helm.sh/release-name=operator-crds
kubectl annotate crd authclients.formance.com meta.helm.sh/release-namespace=formance-system
