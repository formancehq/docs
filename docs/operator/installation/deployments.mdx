...
NAME=$(kubectl get crds -o json  | jq ".items[].metadata.name" --raw-output)

for i in $NAME
do
    echo "----- Annotating $i -----"
    kubectl annotate crd $i meta.helm.sh/release-name=$RELEASE_NAME --overwrite
    kubectl annotate crd $i meta.helm.sh/release-namespace=$NAMESPACE --overwrite
    echo "--------- Done ----------"
done
```
