...
Numscript supports several variable types. Details on the different types and how they are used [are available in the Numscript reference docs](../../reference/numscript/variables.mdx).
...
At least, not at this time. That feature is coming though! In the meantime you _can_ execute Numscript templates using Formance Ledger's built-in API server. If you've never done that before, take a moment to [read up on how to use the API server to execute Numscript](../http/index.mdx).
...
In addition to specifying the value of variables directly, we can pull those values from [metadata](../metadata/index.mdx) associated with accounts.
...
:::tip Dig deeper
Want to learn more about creating Numscript templates? [The Numscript reference docs](../../reference/numscript/variables.mdx) have you covered!
:::
