...
:::info
Accounts in Formance Ledger cannot go negative by default, except for the special `@world` account. For more details on to build transactions that can make accounts go negative, please refer to the [overdraft](/ledger/reference/numscript/overdraft) section.
:::
