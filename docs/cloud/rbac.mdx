...
- Organizations: An organization's role is to manage an organization and stacks. eaz
...
2. Fallback roles are set to organization `GUEST` and stack `GUEST`
   1. A user with _no roles_ and _no stack role_ assigned. would have role `GUEST` and stack role `GUEST` assigned
...
4. Fallback roles are set to organization `NONE` and stack `GUEST`
    ...
    NB: Notice that to access a stack the user **must** have a role set on the organization. None of (4.) would give access to the stack.
