...
After writing the file in a folder, we can launch the deployment with the command :
 ```shell
 docker compose up -d #for Mac and Windows
 docker-compose up -d #for Linux
 ```
## URL
You can access all the services via the following URLs:
```shell
http://localhost/ -> Control UI
http://localhost/api/ledger -> Base Ledger API
http://localhost/api/payments -> Base Payments API
http://localhost/api/search -> Base Search API
http://localhost:8080/dashboard/ -> Traefik Dashboard
http://localhost:5601 -> OpenSearch Dashboard
http://localhost:8081 -> Red Panda Dashboard
```
