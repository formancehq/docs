...
At least, not at this time. That feature is coming though! In the meantime you _can_ execute Numscript templates using Formance Ledger's built-in API server. If you've never done that before, take a moment to [read up on how to use the API server to execute Numscript](/oss/ledger/numscript/http).
...
In addition to specifying the value of variables directly, we can pull those values from [metadata](/oss/ledger/numscript/metadata/) associated with accounts.
