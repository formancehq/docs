...
Once you've run that transaction succesfully, let's have a look at [`leslieknope`'s balance'](https://control.formance.com/accounts/player:leslieknope) to find that transaction. It should be the topmost one listed. Click the details arrow to the right of the transaction to get more details.
...
You can store the value of [template variables](/oss/ledger/numscript/templates/) in metadata as well. Template variables may only be of `monetary`, `account`, or `portion` types, and must be specified like the following examples:
