...
As outlined in the performance model [section](../reference/performance.mdx), we consider the cost of a write to be `O(N) + W`. Let's consider the following example:
