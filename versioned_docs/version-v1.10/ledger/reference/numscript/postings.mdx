...
In Numscript, a [posting](/v1.10/ledger/reference/transactions#postings) models the movement of an amount of an asset from one account to another. Postings are wrapped in a [transaction](/v1.10/ledger/reference/transactions#transactions-1) to ensure that they are applied atomically.
